...
using ZeroTalk.Server.Services;
using ZeroTalk.Shared.Network;

namespace ZeroTalk.Server.Forms
{
    /// <summary>
    ...
    public partial class frmServer : Form
    {
        ...
        private void btnLaunchClients_Click(object sender, EventArgs e)
        {
            try
            {
                string clientExe = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ZeroTalk.Client.exe");
                if (!File.Exists(clientExe))
                {
                    clientExe = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\..\ZeroTalk.Client\bin\Debug\ZeroTalk.Client.exe"));
                }

                if (!File.Exists(clientExe))
                {
                    MessageBox.Show($"ZeroTalk.Client.exe not found at: {clientExe}\nPlease build the solution first.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
